...
class StakeSimple extends Component {
  ...
  getRetirement = () => {
    ...
    if (timeElapsed < 86400) {
      ...
      return {timer: ('0' + Math.trunc(timeLeft / 3600).toString()).slice(-2) 
                ...
                <div style={this.modalStyle} className={classes.paper}>
                  ...
                  <div>
                    <Button
                      ...
                      color="primary"
                      disabled={
                        Number(this.state.unStakeAmount) <= 0 ||
                        Number(this.state.unStakeAmount) > this.yeldStakeAmount ||
                        this.state.unstakeProcessing
                      }
                      onClick={async () => {
                        if (!this.state.unstakeProcessing) {
                          this.setState({ unstakeProcessing: true })
                          dispatcher.dispatch({ type: YELD_UNSTAKE, content: Number(this.state.unStakeAmount) })
                        }
                      }}>
                      <Typography variant={'h5'} color="secondary">
                        {!this.state.unstakeProcessing ? (
